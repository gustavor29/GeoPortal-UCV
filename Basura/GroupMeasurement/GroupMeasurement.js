...
var MeasurePolygon = L.Control.extend({
	options: {
		position: 'topleft',
		...
		html_template: `<p><strong>Area: </strong><br>_p_area</p>
	...
	onAdd: function(map) {
		...
		this._measurePanel = L.control({ position: 'topright' });
...
L.Control.GroupMeasurement = L.Control.extend({
	options: {
		position: 'topleft'
	},
	onAdd: function(map) {
		groupContainerM = L.DomUtil.create('div', 'leaflet-control-group');
		var icon = L.DomUtil.create('img', ' ', groupContainerM);
        icon.src = './dist/GroupMeasurement/ruler_icon.svg'; // Ruta de la imagen SVG
		var tooltip = L.DomUtil.create('div', 'tooltip', groupContainerM);
		tooltip.innerHTML = 'Medición';
		var controls = L.DomUtil.create('div', 'controls', groupContainerM);

		// Agregar subcontroles
		controls.appendChild(new Ruler({unitSystem: 'metric', color: '#D20103'}).onAdd(map));
		controls.appendChild(new MeasurePolygon().onAdd(map));

		// Manejador de mouse para expandir/contraer el grupo de controles
		groupContainerM.addEventListener('mouseenter', function() {
			groupContainerM.classList.add('expanded');
		});
		groupContainerM.addEventListener('mouseleave', function() {
			groupContainerM.classList.remove('expanded');
		});

		return groupContainerM;
	}
});

L.control.groupMeasurement = function(options) {
        return new L.Control.GroupMeasurement(options);
    };